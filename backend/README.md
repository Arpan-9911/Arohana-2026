...
| **Utilities** | nanoid (unique token generation) |
...
const userToken = jwt.sign(payload, process.env.USER_JWT_SECRET);
```
...
USER_JWT_SECRET=<random-32-char-string>
JWT_EXPIRE=7d
...
```
POST   /api/super-admin/society  # Create society
GET    /api/super-admin/societies # List societies (future)
```
...
- [ ] Aadhaar document upload (supports `.pdf`, `.jpg`, `.png`)
